...
var CensusSearchTypeahead = (function() { // DO NOT INJECT FRAMEWORKS OR DEPENDENCIES THAT RELY ON FRAMEWORKS.
    ...
    var initSearchTypeahead = function(searchApiUri, searchResultUrl, isUsingUSASearch) {
    	apiUri = searchApiUri;
        ...
        var callTypeaheadService = function(query) {
            ...
            window.hinterXHR.onreadystatechange = function() {
                if (this.readyState == 4 && this.status == 200) {
                    ...
                    if (result && !isUSASearch) {
                    	...
                    	try {
                    		suggestionResponse = JSON.parse(window.hinterXHR.response);
                    	} catch (e) {}
                    	if (suggestionResponse && suggestionResponse.length) {
	                    	var showResults = false;
	                		var typeaheadHtml = "";
	            			var query = document.getElementById("data-uscb-header-input").value;
	            			var typeaheadObj = document.getElementById("data-uscb-header-search-typeahead");
	                        ...
	                        for (var i = 0; i < suggestionResponse.length; i++) {
	                            var resultItem = suggestionResponse[i];
	                            showResults = true;
	                            typeaheadHtml += '<li><a href="' + resultUrl + resultItem + '">' + resultItem + '</a></li>'
	                        }
	                        
	                        typeaheadHtml += '</ul></div>';
            ...
            var $searchInput = $('#data-uscb-header-input');
            var $form = $searchInput.closest('form');
            var $searchName = $form.find('#usaSearchName');
            var $searchApi = $form.find('#usaSearchApi');
            if ($searchName.length && $searchApi.length) {
            	// format for usa search
            	var url = $('#data-uscb-header-search-typeahead').data("apiurl");
            	window.hinterXHR.open("GET", url + "?name=" + $searchName.val() + "&api_key=" + $searchApi.val() + "&q=" + query, true);
            } else {
