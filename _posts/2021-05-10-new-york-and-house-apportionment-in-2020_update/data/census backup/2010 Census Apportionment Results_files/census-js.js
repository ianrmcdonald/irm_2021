	    ...
	    for (var i = 0; i < fData.length; i++) {
            ...
            if (fData[i].country_selector_type === "aemattachment") {
				listResult += "<td class='uscb-padding-T-20 uscb-padding-B-20' width='" + colWidth + "%'>";
                    ...
                    listResult += "<img alt='' src=" + fData[i].icon_path + " class='uscb-country-list-icon' />";
...
var CensusPagination = (function($, _, digitalData) {   
    ...
    function buildCQ5Item(listItem, i) {
    	...
    	if (i == 0) {
    		cq5ItemHtml += '<hr class="uscb-light-teal-hr uscb-margin-T-0 uscb-margin-B-12"/>';
		}
    	...
    	cq5ItemHtml += '<p class="uscb-list-metadata uscb-sub-heading-2 uscb-color-secondary-1 uscb-line-height-20-18 uscb-capitalize uscb-margin-TB-02">';
		
    	if (listItem.contentType) {
			cq5ItemHtml += listItem.contentType;
		}
    	...
    	if (listItem.myDisplayDate) {
			cq5ItemHtml += listItem.myDisplayDate;
		}
    	
    	if (listItem.myDisplayDate && listItem.showAuthor) {
    		cq5ItemHtml += ' | ';
    	}
    	
    	if (listItem.showAuthor) {    		
    		cq5ItemHtml += listItem.author;
    	}
		...
		cq5ItemHtml += '</p>';
		cq5ItemHtml += '<p class="uscb-list-metadata uscb-sub-heading-2 uscb-color-secondary-1 uscb-uppercase">';
		
		if (listItem.showDocumentNumber) {
			cq5ItemHtml += '<span>' + listItem.documentNumber + '</span>';
		}
		
		if (listItem.showDocumentNumber && listItem.showPrimaryTopicTag) {
			cq5ItemHtml += ' | ';
		}
		
		if (listItem.showPrimaryTopicTag) {
			cq5ItemHtml += '<span>Primary Topic: ' + listItem.primaryTopicTagName + '</span>';
		}
    ...
    function buildTileItems(tileItems, currentPage, componentName) {
		...
		for( var i=0; i < tileItems.length; i++ ) {
			...
			tileItemHtml +=	'<img src="' + tileItems[i].myImageSrc + '" alt="" class="uscb-decorative-img">';
